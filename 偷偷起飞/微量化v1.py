...
class RealtimeMonitor:
    def __init__(self, analyzer, advisor, positions):
        self.analyzer = analyzer
    ...
    def analyze_one(self, code, use_realtime=True):
        ...
        # 用实时行情覆盖最后一根 K 线
        if use_realtime:
            q = RealtimeQuote.fetch([code]).get(code)
            if q and q['price'] > 0:
                df.loc[df.index[-1], 'close'] = q['price']
                df.loc[df.index[-1], 'open'] = q['open'] or df['open'].iloc[-1]
                df.loc[df.index[-1], 'high'] = q['high'] or df['high'].iloc[-1]
                df.loc[df.index[-1], 'low'] = q['low'] or df['low'].iloc[-1]
                df.loc[df.index[-1], 'volume'] = q['volume'] or df['volume'].iloc[-1]
    ...
    def monitor_loop(self, codes, interval=60, rounds=None):
        ...
        try:
            while True:
                ...
                print("=" * 78)
                print(f"📡 实时监控  第 {round_count} 轮  {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
                print("=" * 78)
